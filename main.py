...
def admin_users(_auth=Depends(require_dev_key)):
    """Access tier C: every registered user, with a per-user activity summary
    (screens visited, time spent, buttons clicked, feedback given, recipes
    published) — everything needed for a pilot overview in one call."""
    import json as _json
    conn = get_pg_db()
    try:
        with conn.cursor() as cur:
            ...
            cur.execute(
                "SELECT user_id, COUNT(*), SUM(seconds) FROM screen_time "
                "WHERE screen NOT LIKE %s GROUP BY user_id",
                ('כפתור:%',),
            )
            screen_stats = {r[0]: {"visits": r[1], "seconds": float(r[2] or 0)} for r in cur.fetchall()}

            cur.execute(
                "SELECT user_id, COUNT(*) FROM screen_time WHERE screen LIKE %s GROUP BY user_id",
                ('כפתור:%',),
            )
            click_counts = {r[0]: r[1] for r in cur.fetchall()}

            cur.execute("SELECT user_id, COUNT(*) FROM feedback WHERE user_id IS NOT NULL GROUP BY user_id")
            feedback_counts = {r[0]: r[1] for r in cur.fetchall()}

            cur.execute("SELECT COUNT(*) FROM users WHERE created_at >= now() - interval '7 days'")
            new_this_week = cur.fetchone()[0]

            cur.execute("SELECT data_value FROM shared_data WHERE data_key = 'community_recipes'")
    ...
    if community_row:
        try:
            for r in _json.loads(community_row[0]):
                uid = r.get("authorUserId")
                if uid:
                    recipe_counts[uid] = recipe_counts.get(uid, 0) + 1
        except Exception:
            pass
        ...
        "users": [
            {
                ...
                "screen_visits": screen_stats.get(u[0], {}).get("visits", 0),
                "total_minutes": round(screen_stats.get(u[0], {}).get("seconds", 0) / 60, 1),
                "button_clicks": click_counts.get(u[0], 0),
                "feedback_count": feedback_counts.get(u[0], 0),
                "recipes_published": recipe_counts.get(u[0], 0),
            }
            for u in users
        ],
    }
